...
//            cv::cvtColor(color_img, color_img, CV_BGR2RGB);
            ...
            timer.tic();
            Fusion.integrate(depth, color, T_b_curr);  //输入深度数组float类型的，彩色图像，当前帧到参考基矩阵的转换矩阵
        ...
        if (Fusion.h_exceed_num[1]>10000)
        {
    ...
    std::cout << "new scene, save PCD cost: " << timer.toc() << " ms." << std::endl;
